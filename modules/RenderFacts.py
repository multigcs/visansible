...
class RenderFacts():
								...
								if simple == False:
									...
									if gateway_interface == device:
										if gateway_address not in self.ipv4_ips:
											...
											graph.edge_add("network_" + network, self.ipv4_ips[gateway_address], "color: { color: '" + self.ipv4_ips[gateway_address] + "'}, arrows: {to: true}, label: 'gw:." + gateway_address.split(".")[-1] + "'")
					else:
	...
	def show_host_table_ifaces(self, facts):
		...
		for part in facts:
			if part != "ansible_default_ipv4" and type(facts[part]) is dict and "device" in facts[part]:
				...
				html += bs_table_begin()
				html += self.facts2rows(facts[part], ["device", "model", "macaddress", "mtu", "promisc", "type", "active"])
				html += bs_table_end()
				html += bs_col_end()
				html += bs_col_begin("6")
				if "ipv4" in facts[part]:
					if type(facts[part]["ipv4"]) == list:
						for ipv4 in facts[part]["ipv4"]:
							...
							html += bs_table_begin()
							html += self.facts2rows(ipv4, ["address", "netmask", "broadcast", "network"])
							html += bs_table_end()
							bs_add("<br />")
					else:
						html += bs_add("<b>IPv4:</b>")
						html += bs_table_begin()
						html += self.facts2rows(facts[part]["ipv4"], ["address", "netmask", "broadcast", "network"])
						html += bs_table_end()
						bs_add("<br />")
				if "ipv6" in facts[part]:
					for ipv6 in facts[part]["ipv6"]:
		...
		for device in facts["ansible_devices"]:
			if type(facts["ansible_devices"][device]) is list:
				...
				if "ansible_mounts" in facts:
					...
					for mount in facts["ansible_mounts"]:
						if mount["device"] == "/dev/" + device:
							html += self.facts2rows(mount, ["mount", "fstype", "device", "size_available", "options", "uuid"])
			...
			if "partitions" in facts["ansible_devices"][device]:
				if facts["ansible_devices"][device]["size"] != "0.00 Bytes" and (len(dms) == 0 or not device.startswith("dm-") ):
					...
					html += bs_table_begin()
					html += self.facts2rows(facts["ansible_devices"][device], ["host", "vendor", "model", "serial", "size"])
					## show disk-mounts ##
					if "ansible_mounts" in facts:
						for mount in facts["ansible_mounts"]:
							if "links" in facts["ansible_devices"][device] and "uuids" in facts["ansible_devices"][device]["links"]:
					...
					for partition in facts["ansible_devices"][device]["partitions"]:
						...
						html += bs_table_begin()
						## show partition ##
						html += self.facts2rows(facts["ansible_devices"][device]["partitions"][partition], ["uuid", "size", "start", "sectors", "sectorsize", "drive_letter"])
		...
		if "ansible_lvm" in facts:
			...
			if "vgs" in facts_lvm:
				for vg in facts_lvm["vgs"]:
					...
					html += bs_add("<b>VG:</b>")
					html += bs_table_begin()
					html += self.facts2rows(facts_lvm["vgs"][vg], ["size_g", "free_g", "num_lvs", "num_pvs"])
					if "pvs" in facts_lvm:
						for pv in facts_lvm["pvs"]:
							...
							if pv_vg == vg:
								...
								html += bs_add(" <td>" + pv + "</td>")
								html += bs_add("</tr>")
								html += self.facts2rows(facts_lvm["pvs"][pv], ["size_g", "free_g"], "&nbsp;&nbsp;&nbsp;")
								html += bs_add("<tr><td>&nbsp;</td><td>&nbsp;</td></tr>")
					html += bs_table_end()
					html += bs_col_end()
					html += bs_col_begin("6")
					if "lvs" in facts_lvm:
						for lv in facts_lvm["lvs"]:
							...
							if lv_vg == vg:
								...
								html += bs_table_begin()
								html += self.facts2rows(facts_lvm["lvs"][lv], ["size_g"])
								## show mounts ##
								if "ansible_mounts" in facts:
									for mount in facts["ansible_mounts"]:
										if mount["device"] == lv_device:
	...
	def show_host_table_general(self, facts):
		...
		html += bs_col_begin("6")
		html += bs_table_begin()
		html += self.facts2rows(facts, ["ansible_fqdn", "ansible_system_vendor", "ansible_product_name", "ansible_product_serial", "ansible_architecture", "ansible_memtotal_mb", "ansible_virtualization_role", "ansible_virtualization_type"])
		html += bs_table_end()
		html += bs_col_end()
		html += bs_col_begin("6")
		html += bs_table_begin()
		html += self.facts2rows(facts, ["ansible_distribution", "ansible_distribution_major_version", "ansible_distribution_release", "ansible_distribution_version", "ansible_distribution_file_variety", "ansible_userspace_architecture", "ansible_kernel", "ansible_pkg_mgr"])
		html += bs_table_end()
		html += bs_col_end()
		html += bs_row_end()
		html += bs_row_begin()
		...
		html += bs_table_begin()
		html += self.facts2rows(facts, ["ansible_processor_count", "ansible_processor_cores", "ansible_processor_threads_per_core", "ansible_processor_vcpus"], "", "", "right")
		html += bs_table_end()
		html += bs_col_end()
		html += bs_col_begin("6")
	...
	def show_host_table_memory_hist(self, facts, stamp, hostname):
		...
		if "ansible_memory_mb" in facts:
			...
			for section in ["nocache", "real", "swap"]:
				...
				data = []
				for timestamp in sorted(set(stamps)):
					if hostname in self.inventory["hosts"] and timestamp in self.inventory["hosts"][hostname]:
		...
		if "ansible_memory_mb" in facts:
			...
			for section in ["nocache", "real", "swap"]:
				if section in facts["ansible_memory_mb"]:
					...
					html += bs_table_begin()
					html += self.facts2rows(facts["ansible_memory_mb"][section], "", "", "MB", "right")
					html += bs_table_end()
					html += bs_col_end()
			html += bs_row_end()
			html += bs_card_end()
	...
	def show_host_table_network(self, facts):
		...
		html += bs_table_begin()
		html += self.facts2rows(facts, ["ansible_hostname", "ansible_domain", "ansible_fqdn"])
		html += bs_table_end()
		html += bs_col_end()
		html += bs_col_begin("6")
		...
		html += bs_table_begin()
		if "ansible_default_ipv4" in facts:
			html += self.facts2rows(facts["ansible_default_ipv4"], ["gateway", "interface"])
		html += bs_table_end()
		html += bs_col_end()
		html += bs_row_end()
		html += bs_card_end()
	...
	def show_history(self, stamp = "0",  hostname = ""):
		...
		if hostname != "":
			if self.inventory["hosts"][hostname]["first"] != "0":
				last = 0
				for timestamp in range(int(self.inventory["hosts"][hostname]["first"]), laststamp):
					if str(timestamp) in self.inventory["hosts"][hostname]:
						if "ansible_facts" in self.inventory["hosts"][hostname][str(timestamp)]:
							last = 1
						else:
							last = 0
					if timestamp >= int(self.inventory["hosts"][hostname]["first"]) and timestamp % tsteps == 0:
						data.append(last)
				datas.append(data)
					...
					for hstamp in sorted(self.inventory["hosts"][host]):
						if hstamp != "0" and hstamp.isdigit():
							if int(timestamp) >= int(hstamp):
								if "ansible_facts" in self.inventory["hosts"][host][hstamp]:
									last = 1
									last2 = 1
								else:
									last = 0
	...
	def show_hostdata(self, host, stamp = "0"):
		...
		if stamp == "0":
			...
			html = HtmlPage("Visansible <small>(" + str(len(self.inventory["hosts"])) + " hosts)</small>", "Host (" + host + ") <a href='rescan?host=" + host + "'>[RESCAN]</a>" + groups, datetime.fromtimestamp(int(stamp)).strftime("%a %d. %b %Y %H:%M:%S") + "", links);
		if stamp in self.inventory["hosts"][host] and "ansible_facts" in self.inventory["hosts"][host][stamp]:
			...
			if "0" in self.inventory["hosts"][host] and "msg" in self.inventory["hosts"][host]["0"]:
	...
	def show_csv(self):
		...
		for host in self.inventory["hosts"]:
			invHost = self.inventory["hosts"][host]
			ipaddr = ""
			if "0" in self.inventory["hosts"][host] and "ansible_facts" in self.inventory["hosts"][host]["0"]:
				invHostLatest = invHost["0"]
				invHostLatestFacts = invHostLatest["ansible_facts"]
				for part in invHostLatestFacts:
					if part != "ansible_default_ipv4" and type(invHostLatestFacts[part]) is dict and "device" in invHostLatestFacts[part]:
...
#							data += prefix + "    " + option + ": " + str(self.inventory["hosts"][host]["options"][option]) + "\n"
	...
	def show_hosts(self, stamp = "0", sgroup = "all", search = ""):
		...
		if stamp == "0":
			html = HtmlPage("Visansible <small>(" + str(len(self.inventory["hosts"])) + " hosts)</small>", "Hosts (" + sgroup + ")", "latest info", self.show_history(stamp));
		else:
			html = HtmlPage("Visansible <small>(" + str(len(self.inventory["hosts"])) + " hosts)</small>", "Hosts (" + sgroup + ")", datetime.fromtimestamp(int(stamp)).strftime("%a %d. %b %Y %H:%M:%S"), self.show_history(stamp));
		stamps = []
		if self.livestatus != "":
			...
			if sgroup == "all" or sgroup == group:
				if search == "":
					...
					for section in self.inventory["groups"][group]["options"]:
						html.add(" Vars: ")
						if type(self.inventory["groups"][group]["options"][section]) == dict:
							for var in self.inventory["groups"][group]["options"][section]:
								html.add("<small>" + section + ":" + var + "=" + self.inventory["groups"][group]["options"][section][var] + " </small>")
						elif type(self.inventory["groups"][group]["options"][section]) == str:
							html.add("<small>" + section + ":" + self.inventory["groups"][group]["options"][section] + " </small>")
						else:
							for var in self.inventory["groups"][group]["options"][section]:
								...
								if "tags" in issue:
									for tag in issue["tags"]:
										if tag["name"] == "server:" + host:
											issue["match"] = True
											tickets += 1
										elif "0" in invHost and "ansible_facts" in invHost["0"] and "ansible_fqdn" in invHost["0"]["ansible_facts"] and tag["name"] == "server:" + invHost["0"]["ansible_facts"]["ansible_fqdn"]:
											tickets += 1
							if tickets > 0:
	...
	def show_inventory(self):
		html = HtmlPage("Visansible <small>(" + str(len(self.inventory["hosts"])) + " hosts)</small>", "Inventory-File", "");
		html.add(bs_row_begin())
		html.add(bs_col_begin("12"))
	...
	def show_stats(self, stamp = "0"):
		...
		if stamp == "0":
			html = HtmlPage("Visansible <small>(" + str(len(self.inventory["hosts"])) + " hosts)</small>", "Stats", "latest info", self.show_history(stamp));
		else:
			html = HtmlPage("Visansible <small>(" + str(len(self.inventory["hosts"])) + " hosts)</small>", "Stats", datetime.fromtimestamp(int(stamp)).strftime("%a %d. %b %Y %H:%M:%S"), self.show_history(stamp));
		html.add(bs_row_begin())
		for option in options:
			html.add(bs_col_begin("4"))
