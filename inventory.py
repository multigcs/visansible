				...
				if parent == "host":
					...
					if part not in self.inventory["hosts"]:
						...
						for hostname in hostnames:
							...
							self.inventory["hosts"][hostname]["maingroup"] = path.split("/")[-1]
							for group in path.split("/"):
								if group != "":
									self.inventory["hosts"][hostname]["groups"].append(group)
				elif parent == "vars":
					group = path.split("/")[-1]
			...
			for line in hostslist.split("\n"):
				if line.startswith("#"):
					...
					if "=" in line:
						...
						self.inventory["groups"][group]["options"][section][name] = value
					else:
						if section not in self.inventory["groups"][group]["options"]:
							self.inventory["groups"][group]["options"][section] = []
						self.inventory["groups"][group]["options"][section].append(line.strip())
				...
				for filename in hists:
					...
					if os.path.isfile("./facts/hist_" + str(stamp) + "/" + host):
						with open("./facts/hist_" + str(stamp) + "/" + host) as json_file:
							...
							if "ansible_facts" in hostdata:
								self.inventory["hosts"][host]["info"] += "OK:" + datetime.fromtimestamp(int(stamp)).strftime("%H:%M:%S") + " "
							else:
								self.inventory["hosts"][host]["info"] += "ERR:" + datetime.fromtimestamp(int(stamp)).strftime("%H:%M:%S") + " "
