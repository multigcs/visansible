...
class VisGraph:
    def __init__(self, name="visjsgraph", setup={}, height="640px"):
        ...
        if self.type == "vis.js":
            self.html += (
                "<div id='" + self.name + "' style='height: " + height + ";'></div>\n"
            )
            self.html += "<script>\n"
    ...
    def end(self, direction=""):
        ...
        if self.type == "vis.js":
            ...
            self.html += (
                "  	if (nodeid.startsWith('host_') && ! nodeid.includes('_ipv4_')) {\n"
            )
            ...
            if direction != "":
                ...
                self.graph.close()
                ret = os.popen(
                    self.type + ' -T svg "/tmp/' + self.name + '.dot"'
                ).read()
            # 			self.html += "<script>\n"
    ...
    def node_add(self, nid, label, icon="desktop-tower", options=""):
        ...
        if self.type == "vis.js":
            ...
            if not os.path.isfile(icon + ".png"):
                ret = os.popen(
                    "convert -scale 32x " + icon + " " + icon + ".png"
                ).read()
            icon += ".png"
            link = ""
            ...
            self.graph.write(
                ...
                + '" [shape="none",tooltip="'
                + label
                + '",label=<<table border=\'0\' cellspacing=\'0\'><tr><td width="100" height="50" fixedsize="true"><IMG src=\''
                + icon
                + "' /></td></tr><tr><td>"
                + label.replace("\\n", "<br />")
                + '</td></tr></table>>, URL="'
