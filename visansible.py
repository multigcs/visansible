    ...
    def do_GET(self):
        ...
        if self.path.startswith("/rescan"):
            ...
            command = [
                "ansible",
                "-i",
                self.rf.inventory["file"],
                opts["host"],
                "-m",
                "setup",
                "--tree",
                "facts/hist_" + str(timestamp),
            ]
            ...
            for host in self.inventory["hosts"]:
                ...
                if "0" in invHost and "msg" in invHost["0"]:
                    ...
                    errors = 2
                os.system(
                    "cp -a facts/hist_" + str(timestamp) + "/" + host + " facts/" + host
                )
            ...
            if errors <= 1:
                ...
                if opts["host"] in self.inventory["hosts"]:
                    html.add(
                        " setTimeout(function() {location.href = '/host?host="
                        + opts["host"]
                        + "'}, 2000);\n"
                    )
                elif opts["host"] in self.inventory["groups"]:
                    html.add(
                        " setTimeout(function() {location.href = '/hosts?group="
                        + opts["host"]
                        + "'}, 2000);\n"
                    )
                else:
                    html.add(
                        " setTimeout(function() {location.href = '/hosts'}, 2000);\n"
                    )
                html.add("</script>\n")
            ...
            if "search" not in opts:
                opts["search"] = ""
            self.write_data(
                self.rf.show_hosts(opts["stamp"], opts["group"], opts["search"])
            )
            return
...
def run():
    ...
    print(
        "starting server (http://" + vasetup["ip"] + ":" + str(vasetup["port"]) + ")..."
    )
